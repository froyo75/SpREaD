...
if [[ $# -eq 0 || -z "$1" ]]; then
	...
	createfleet)
		if [[ -z "$2" || -z "$3" || -z "$4" ]]; then
			...
			axiom-fleet ${FLEET_NAME} -i ${NB_INSTANCES} -r ${REGIONS}
	...
	getinfosfleet)
		if [[ -z "$2" || -z "$3" ]]; then
			...
			axiom-select ${FLEET_NAME}\*

			echo -e "\e[32m[+] Retrieving information from instances...\e[0m"
			axiom-scp ${GETINFOS_SCRIPT_FILEPATH} "${FLEET_NAME}*:getinfos.sh"
			axiom-exec 'chmod +x getinfos.sh && ./getinfos.sh' -q -t
			axiom-scp "${FLEET_NAME}*:/home/op/info-fleet/*" ${OUTPUT_FOLDER}/infos-fleet-recon/
			echo -e "\e[32m[+] Done.\e[0m"
	...
	recon)
		if [[ -z "$2" || -z "$3" || -z "$4" ]]; then
			...
			axiom-scan ${DOMAIN_LIST} -m tlsx-san-cn -anew ${OUTPUT_FOLDER}/subdomains/all_doms_tlsx_subs.txt --quiet --rm-logs 1>/dev/null
			...
			axiom-scan ${DOMAIN_LIST} -m subfinder -anew ${OUTPUT_FOLDER}/subdomains/all_subfinder_subs.txt --quiet --rm-logs 1>/dev/null
            ...
            if [[ ${nb_cidr_list} > 0 ]]; then
                ...
                axiom-scan ${CIDR_LIST} -m dnsx-ptr -anew ${OUTPUT_FOLDER}/subdomains/all_reverse_subs.txt --quiet --rm-logs 1>/dev/null
                ...
                axiom-scan ${CIDR_LIST} -m tlsx-san-cn -anew ${OUTPUT_FOLDER}/subdomains/all_cidrs_tlsx_subs.txt --quiet --rm-logs 1>/dev/null
            ...
            if [[ ${nb_subdoms} > 0 ]]; then
                axiom-scan ${OUTPUT_FOLDER}/subdomains/all_subs.txt -m dnsx-a-re -anew ${OUTPUT_FOLDER}/subdomains/all_active_subs.txt --quiet --rm-logs 1>/dev/null
            fi
			...
			for active_sub_entry in $(cat ${OUTPUT_FOLDER}/subdomains/all_active_subs.txt);do
				...
				ip=$(echo ${active_sub_entry} | awk -F ' ' '{print $2}' | tr -d '[]')
			...
			axiom-scan ${OUTPUT_FOLDER}/ips/all_scans_ips_only.txt -m naabu-full -anew ${OUTPUT_FOLDER}/services/all_scans_ips_top_ports_full.txt --quiet --rm-logs 1>/dev/null
			...
			axiom-scan ${OUTPUT_FOLDER}/subdomains/all_scans_subs_only.txt -m naabu-full -anew ${OUTPUT_FOLDER}/services/all_scans_subs_top_ports_full.txt --quiet --rm-logs 1>/dev/null
			...
			axiom-scan ${OUTPUT_FOLDER}/services/all_scans_subs_ips_top_ports_full_randomized.txt -m httpx -anew ${OUTPUT_FOLDER}/http/all_active_http.txt --quiet --rm-logs 1>/dev/null
			...
			if [[ ${NMAP_SCAN} = NMAP_SV_SCAN ]]; then
				...
				for active_port in "${all_active_ports[@]}"; do
					...
					axiom-scan ${OUTPUT_FOLDER}/services/nmap/${active_port}_IN_PROGRESS -m nmapx -p${active_port} -sTVC -Pn -T3 --max-rtt-timeout 2 --min-parallelism 5 --max-rate 1000 -oA ${OUTPUT_FOLDER}/services/nmap/SV_TCP_${active_port} --rm-logs 1>/dev/null
				...
				axiom-scan ${OUTPUT_FOLDER}/services/all_scans_subs_ips.txt -m nmapx -pU:161,162,123,53,500,137,111,67,68,69 --open -sUVC -Pn -T3 --max-rtt-timeout 2 --min-parallelism 5 --max-rate 1000 -oA ${OUTPUT_FOLDER}/services/nmap/SV_UDP_ALL_SUBS_IPS --rm-logs 1>/dev/null
				echo -e "\e[32m[+] Done.\e[0m" | tee -a ${OUTPUT_FOLDER}/logs/${ACTION}_${LOG_FILEPATH}
			fi
		fi
	;;
	gau)
	  if [[ -z "$2" || -z "$3" || -z "$4" ]]; then
			...
			axiom-scan ${DATA_LIST} -m gau2 -anew ${OUTPUT_FOLDER}/gau/all_gau_urls.txt --quiet --rm-logs 1>/dev/null
	...
	gowitness)
	  if [[ -z "$2" || -z "$3" || -z "$4" ]]; then
			...
			axiom-scan ${DATA_LIST} -m gowitness2 -o ${OUTPUT_FOLDER}/gowitness/screenshots --quiet --rm-logs 1>/dev/null
			nb_screenshots=$(ls -1 ${OUTPUT_FOLDER}/gowitness/screenshots/merge | wc -l)
	...
	nuclei)
	  if [[ -z "$2" || -z "$3" || -z "$4" || -z "$5" ]]; then
			...
			light)
				...
				#axiom-scan ${DATA_LIST} -m nuclei -o ${OUTPUT_FOLDER}/nuclei/non_intrusive_nuclei.txt --quiet --rm-logs 1>/dev/null
				axiom-scan ${DATA_LIST} -m nuclei -exclude-templates cnvd/ -exclude-templates default-logins/ -exclude-templates fuzzing/ -exclude-templates vulnerabilities/ -exclude-templates cves/ -etags oob,intrusive,generic -stats -c 50 -rl 300 -nc -o ${OUTPUT_FOLDER}/nuclei/non_intrusive_nuclei.txt --quiet --rm-logs 1>/dev/null
			;;
			intrusive)
				createFolder ${OUTPUT_FOLDER}/nuclei
				...
				axiom-scan ${DATA_LIST} -m nuclei -exclude-templates fuzzing/ -etags intrusive -stats -c 50 -rl 300 -nc -o ${OUTPUT_FOLDER}/nuclei/intrusive_nuclei.txt --quiet --rm-logs 1>/dev/null
			;;
			*)
