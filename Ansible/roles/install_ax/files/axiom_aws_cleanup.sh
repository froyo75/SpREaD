...
if [[ -f "$AXIOM_PATH/axiom.json" ]]; then
    sec_group_id=$(aws ec2 describe-security-groups --filters "Name=group-name,Values=$name" --query 'SecurityGroups[0].GroupId' --output text)
...
echo -e "\e[32m[*]  Cleanup security group and images associated with the Ax profile..."
